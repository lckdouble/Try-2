...
class EncoderImagePrecomp(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, no_imgnorm=False):
        ...
        # img_dim: Dimensionality of the image embedding
        # embed_size: Dimensionality of the joint embedding
...
class EncoderText(nn.Module):
    ...
    def __init__(self, vocab_size, word_dim, embed_size, num_layers,
        ...
        self.embed_size = embed_size
        # whether normalize the text embedding or not
    ...
    def forward(self, x, lengths):
        ...
        # batch 中的 元素以它们长度的逆序排列
        padded = pad_packed_sequence(out, batch_first=True)
        # 填充后的序列 batch中序列的长度列表
        cap_emb, cap_len = padded
        # 因为是 bidirection, 所以这样处理
        if self.use_bi_gru:
...
class GSMN():
    ...
    def __init__(self, opt):
        ...
        # feat_dim: Dimensionality of the similarity embedding
        ...
        if torch.cuda.is_available():
            ...
            # 如果网络的输入数据维度或类型变化不大，设置这个FLAG
            # 可以让内置的cuDNN的 auto-tuner 自动寻找最适合当前配置
            # 的高效算法来优化运行效率的问题
            # 但是当网络的输入数据在每次 iteration 都变化的话，会导致
            # cnDNN每次都会寻找一遍最后配置，这样反而会降低运行效率
